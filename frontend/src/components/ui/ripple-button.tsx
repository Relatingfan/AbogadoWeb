...
const RippleButton: React.FC<RippleButtonProps> = ({
  ...
  rippleDuration = 600,
  hoverBaseColor = '#2A6B2A', // default GC Global green
  hoverRippleColor: customHoverRippleColor,
  hoverBorderEffectColor = '#C9A84C77', // default gold ripple border
  ...
  const createJsRipple = (event: MouseEvent<HTMLButtonElement | HTMLAnchorElement>) => {
  ...
  if (variant === 'hover') {
    ...
    if (href) {
      return (
        <>
          ...
          <a className={hoverButtonFinalClassName} href={href} onClick={handleButtonClick}>
    ...
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <style dangerouslySetInnerHTML={{ __html: dynamicGridHoverStyles }} />
        <button className={hoverButtonFinalClassName} onClick={handleButtonClick} disabled={disabled} type={type}>
          <span className="relative z-[10] pointer-events-none">{children}</span>
          {jsRippleElements}
          <div
            className="hover-variant-grid-container absolute inset-0 grid overflow-hidden pointer-events-none z-[0]"
  ...
  if (variant === 'ghost') {
    ...
    if (href) {
      return (
        <>
          <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
          <a className={ghostButtonFinalClassName} href={href} onClick={handleButtonClick}>
            <span className="relative z-10 pointer-events-none">{children}</span>
            {jsRippleElements}
          </a>
        </>
    ...
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <button className={ghostButtonFinalClassName} onClick={handleButtonClick} disabled={disabled} type={type}>
          <span className="relative z-10 pointer-events-none">{children}</span>
          {jsRippleElements}
        </button>
      </>
  ...
  // Default variant
  const baseClasses = "relative border-none overflow-hidden isolate transition-all duration-200 cursor-pointer px-4 py-2 bg-[#2A6B2A] hover:opacity-90 text-white rounded-lg";
  ...
  if (href) {
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <a className={buttonClasses} href={href} onClick={handleButtonClick}>
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
      <button className={buttonClasses} onClick={handleButtonClick} disabled={disabled} type={type}>
        <span className="relative z-[1] pointer-events-none">{children}</span>
        {jsRippleElements}
      </button>
    </>
